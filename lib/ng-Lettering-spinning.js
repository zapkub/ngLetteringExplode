	...
	return {
		...
		controller:function($scope, $element, $attrs ,$compile ,$window,$timeout){
			...
			function injector( element, splitter, klass, after  ) {
				...
				if( a.length ) {
					angular.forEach( a, function( item, i ) {
						...
						inject += '<span ng-style="charStyle.'+$scope.groupname+'['+(i)+']" class="' + klass + (i + 1) + '">' + item + '</span>' + after;
					});
			...
			function setRandomizedPosition(){
				...
				console.log($scope.charStyle)
				if (!$scope.charStyle[$scope.groupname]){

					$scope.charStyle[$scope.groupname] = [ ]
				}
				angular.forEach($element[0].childNodes,function(item ,i){
					...
					$scope.charStyle[$scope.groupname][i] = {
		...
		link: function( scope, element, attrs  ) {
			...
			scope.$watch(attrs.unrotate, function (show) {
				...
				if(show){
					...
					spinBackAll(scope,attrs.groupname)
				}else {
					spinRandAll(scope,attrs.groupname)
				}
			});
		}
	}
...
function spinCharToZero($scope,i){
	for (var j = i - 15;j<i+15;j++){
		...
		$scope.charStyle[$scope.groupname][j] = {
...
function spinBackAll(scope,groupname){
	angular.forEach(scope.charStyle[groupname],function(item ,i){
		var degree = 0;
		var scale = 1;
		scope.charStyle[groupname][i] = {
			transition: transitionIn(),
...
function spinRandAll (scope,groupname){
...
function spinToRand($scope,i,groupname){
	if(i){
		for (var j = i - 15;j<i+15;j++){
			var degree = Math.round( Math.random() * 30 ) * signage[ Math.round(Math.random() * 1) ]
